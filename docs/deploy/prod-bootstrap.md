...
>
> ⚠️ **Database I/O:** `rebuild` перестраивает B-дерево агрегата — доминирующая статья
> расхода I/O (backlog «rebuild — снизить Database I/O»). Прогнать `rebuild` **по одному
> разу на раскладку**; `ladderReport` — read-only, гоняй сколько нужно. На Free-плане
> лимит I/O — 1 ГБ/мес (месячный сброс 1-го числа), заливку планировать на начало месяца.
