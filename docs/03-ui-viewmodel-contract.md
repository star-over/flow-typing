...
*   Для каждого `TARGET` пальца, его `keyCapStates` должен содержать **полный набор** всех клавиш, за которые этот палец отвечает согласно `finger-layout-asdf.json`. Это необходимо, чтобы UI мог построить целостную визуальную картину рабочего пространства пальца.

#### 4. Навигационные подсказки (`navigationRole`)
...
*   Направление движения роль не несёт: его показывает анимация `MovementPath` по `navigationPath` (п. 6). Дискретных стрелок на клавишах нет — ADR 0027.
...
*   Строится в конвейере (`applyNavigationPaths`, `hands-scene.ts`) той же парой `getHomeKeyForFinger` + `findOptimalPath`, что раскидывает пер-клавишные роли (п. 4). Это **упорядоченная форма того же пути**, отдаваемая потребителю целиком, — чтобы UI рисовал движение по готовому массиву и **не пересобирал граф**.
...
*   **Отношение к п. 4:** разбросанные пер-клавишные `navigationRole: 'PATH'` **остаются** (их читает `KeyboardScene` — на них держится кольцо клавиш пути); `navigationPath` — упорядоченная форма того же пути для анимации `MovementPath`. Направление несёт только анимация: дискретные стрелки упразднены (ADR 0027), поэтому второго представления направления в сцене нет.
