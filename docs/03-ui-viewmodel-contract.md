...
Этот документ является формальной спецификацией для объекта `HandsSceneViewModel` — структуры данных, которая служит "визуальным контрактом" между бизнес-логикой приложения (машинами состояний) и UI-компонентами (`HandsExt.svelte`).
