...
|---------|-----------|-------------------------|
| **Объединение в группы** | Объединение элементов в осмысленные блоки | Группировка клавиш по пальцам/рядам, а не по отдельности |
...
1. **Домашний ряд — не единственный старт.** В текущей `KeyLadder` для йцукен шаг 0 уже включает указательные пальцы по всем трём рядам. Это оправдано: указательные — сильнейшие пальцы, и их раннее включение ускоряет формирование групп паттернов «домашний ряд + верх + низ».
...
|-------|------------------------|-------------------|
| **Длина слова** | Длинные слова требуют удержания большей последовательности в памяти | Независимая от шага |
...
**Типовое применение:**
- Кредитная оценка: «одобрено / отказано / на ручную проверку».
...
- Распределение по приоритетам: критичный → сразу, обычный → в очередь.
...
- Чек-листы соответствия требованиям.
...
- Ядро написано на Rust, привязки доступны для Node.js и TypeScript.
