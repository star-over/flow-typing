...
**Что:** Сейчас `SESSION.COMPLETE` срабатывает per-drill: одна тренировка = один drill = одно событие завершения. Полноценная семантика «сессия» (несколько drill'ов подряд, агрегированные WPM/accuracy/errorCount по всем drill'ам, явное завершение сессии пользователем) не реализована. Внутреннее состояние `training.machine` уже намекает на это разделение (комментарий в `src/machines/training.machine.ts:126-128`: «`lessonComplete` — деталь реализации, `SESSION.COMPLETE` — событие сессионного слоя»), но второго слоя над `lesson` нет.

**Почему отложено:** Текущий «одна сессия = один drill» работает для MVP. Без `/stats` (Phase 7) и без адаптивной системы (`docs/05`) нет потребителя агрегата на уровне сессии.

**Driver:** Phase 6 (sessions tracking) или Phase 7 (`/stats`) — в момент, когда наружу понадобится «средний WPM за сессию» / «общая длительность сессии». Тогда сессионный слой обретёт consumer'а.

**Update 2026-06-12:** семантика сессии зафиксирована на сессии проектирования адаптивной выдачи (см. CONTEXT.md → Session): Session = ограниченный таймером отрезок, длительность — настройка пользователя, drill'ы подаются до истечения таймера, завершение — на границе текущего drill'а. Адаптивный движок — третий драйвер рефакторинга (наряду с Phase 6/7): без таймер-сессии у потоковой подачи порций нет рамки.

**Размер:** средняя фаза — структурный рефакторинг машины + новое имя события / новый child actor + миграция существующих тестов.
