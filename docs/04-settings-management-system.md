...
2. **Единый хранилище, два UI-входа:** все настройки лежат в одном хранилище `settings`. Видимо они разделены на две роли — *настройки приложения* и *настройки тренажёра* — и редактируются на разных экранах:
   - **Приложение** (`/settings` — `SettingsPage.svelte`): `interfaceLanguage`, `theme`, `displayName`. Метаданные UI: «как со мной разговаривает приложение».
   - **Тренажёр** (`MenuScreen` на `/train`): `textLanguage`, `symbolLayoutId`, `fingerLayoutId`, курсор FlowLine (`cursorType`/`cursorMode`), `rhythmChannelEnabled`. «Что и на чём тренируем» — pre-flight controls перед стартом сессии.
