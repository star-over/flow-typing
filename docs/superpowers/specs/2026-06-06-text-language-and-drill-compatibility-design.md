...
- Классификация drills по темам (литература / код / новости и т.п.).
...
- Потоковая обработка корпуса из БД (этап 2 из `docs/05`) — формат JSONL подготавливается для этого, но миграция на бэкенд не делается.
...
**Отвергнуто:** «пропускать несовместимые символы посередине» — это создаёт «дыры» в тексте (пользователь не видит закономерности, особенно при смешанных или экзотических раскладках, например украинский текст в кириллической раскладке без специфичных букв).
...
```
interfaceLanguage (всегда задан, жёстко заданный дефолт)
       │
...
// ↑ `currentTextLanguage` НЕ нужен — язык извлекается из descriptor'а раскладки
...
После прогона:
- `drills.json` удаляется из репозиторий.
- `drills.jsonl` коммитится.
- Лог скрипта (сколько перенесено, сколько выброшено и какие) включается в PR.
...
Модуль рендеринга настройки разрешает функцию-источник, передавая ей актуальные preferences.

### 7.3. Первая загрузка нового пользователя

Без адаптации, без модальных окон. `localStorage` пуст → `normalizePreferences` возвращает каскад дефолтов. Главный экран открывается сразу, пользователь жмёт «Старт» и попадает в совместимую тренировку.
...
Инварианты 1–4, 6–8 проверяются и в production (при загрузке модуля), и в тестах (через те же схемы), без дублирования.
