...
- **Готовая библиотека (shadcn и аналоги).** На момент этой спецификации отложена: тогда её
  брали как React-only и вне стека проекта; доменные вопросы (CTA-ссылка, `KeyHint`,
  i18n, темы) она не закрывает в любом случае. Смена курса — отдельным ADR, не через
  кнопку; позже так и вышло: `shadcn-svelte` (Bits UI + Tailwind v4) работает на
  Svelte 5 без React, хром переведён на неё в ADR 0035.
