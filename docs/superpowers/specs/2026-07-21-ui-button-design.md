...
сущности. Разбор в брейнсторме выделил шесть развилок (граница компонента;
вариант против отдельного компонента; отношение к `Surface`; полиморфизм
`button`/`a`; граница «стиль компонента ↔ лейаут владельца»; API контента) и
...
Компонент покрывает **только интерактивные контролы**. Пассивные плашки
(будущий `ui/Surface`), сегмент тогл-группы (будущий `ui/SegmentedControl`) и
каретка логотипа (глиф, не контрол) — вне границы.
...
- **Полиморфизм элемента.** Корень — `<svelte:element this={href ? 'a' : 'button'}>`.
  ...
  вкладке, preload маршрута, семантика для скринридера), поэтому моделируется
...
- **Контент — сниппет `children`** (`{@render children()}`): кнопки оборачивают
...
- **Реконсиляция текста плотных вариантов.** Сейчас `FooterActions` красит текст
...
### Граница «стиль компонента ↔ лейаут владельца»
...
| `SurveyPrompt` `.answer` | `variant="neutral"` + локальный оверрайд фона на `--color-background` (кнопки лежат внутри surface-карточки) |
...
«стиль компонента ↔ лейаут владельца». Тело — краткое, детали — в этом спеке.
...
- **Матрица `style × tone`** вместо семантических имён. Даёт комбинации, которых
  в продукте нет (`ghost` + `danger`). Выбраны семантические имена в один проп
  `variant` (читаемее на call-site, YAGNI).
- **Втягивание сегмента и каретки.** `SessionDurationSelector .segment` — член
  тогл-группы (selected-состояние, `flex:1`), будущий `ui/SegmentedControl`.
...
- **`ui/Surface`** (консолидация surface-плашек) — отдельный спек. Button делит
  с Surface только роль-токены, но не код; смешивать контейнер и контрол в один
  примитив не станем.

## Область неопределённости

- **Реконсиляция foreground → `--color-on-dense`** и **оверрайд фона `.answer`**
