...
Критерий приёмки по контрасту (DESIGN.md требует WCAG ≥4.5:1; прецедент — коммит `77faccd` чинил контраст метки): пройтись по темам (light/sepia/dark/nord — переключатель темы в тулбаре Storybook) и **измерить** контраст `--color-text-muted` на `--color-surface-accent` в каждой (devtools → Styles → вычисленные значения → онлайн-калькулятор контраста). `--color-text-muted` — самая приглушённая роль, при `font-size-xs` это худший сценарий, оценки глазами недостаточно. Если хотя бы в одной теме <4.5:1 — заводить роль `--color-kbd-*` по алгоритму `docs/06` §6.7 (запись в `ROLE_DICTIONARY` + значения во всех темах + `unset` в `_template.css`) и остановиться для согласования.
