...
- [ ] **Step 3: sepia.css** — комментарий «Описание контракта — см. light.css» → «Описание контракта — `docs/06-component-contracts-and-themes.md`». И у `--wordmark-caret-color` (~строка 486) убрать «(решение 9)» из комментария: отсылка к нумерации плана недолговечна, комментарий «тёмный на янтаре во всех темах» самодостаточен.
