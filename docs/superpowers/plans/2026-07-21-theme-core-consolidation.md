...
Перевод OKLCH → линейный sRGB остаётся приватной функцией модуля: наружу его никто не зовёт, а лишний экспорт `make knip` пометит мёртвым.
...
- [ ] **Step 3: Обновить шапку файла — слоёв два, не три**

Шапка `sepia.css` всё ещё описывает три слоя, хотя слой контракта растворён (ADR 0029) — `_template.css` давно исправлен, а `sepia` отстал. Заменить строку

```
 * ТРИ СЛОЯ (эталон схемы; все темы на ней):
```

на

```
 * ДВА СЛОЯ (ADR 0029; эталон схемы — все темы на ней):
```

и удалить пункт 3 целиком (абзац «3. КОНТРАКТ КОМПОНЕНТОВ — …» вплоть до строки перед «Правило границы»). Пункты 1 и 2 не трогать.

- [ ] **Step 4: Проверить, что имя `--route-` в теме не осталось**
...
| `--bead` (0.88 0 0) ↔ `--primary-hover`/`--color-primary-hover` (0.88 0.01 234) | 0.0100 | `--color-marker-core` ↔ `--color-primary-hover` |
...
`--bead` ↔ `--color-primary-hover` не сводится: `--bead` ахромат (c=0), `--color-primary-hover` — цветной (c=0.01, h=234); свести значило бы обнулить хрому литералом в формуле роли, то есть связь была бы фиктивной, не выводом. ΔE 0.0100 мал, но это совпадение координат при разной природе токенов (нейтральное тело бусины vs цветная ветка 254°), не общий источник — решение не сводить.
