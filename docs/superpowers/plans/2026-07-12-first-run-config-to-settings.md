...
**Goal:** Убрать конфиг-форму из пяти выпадающих списков с пути между «Начать тренировку» и первым нажатием: `MenuScreen` сводится к одиночной CTA, шесть тренировочных элементов управления переезжают на `/settings`.

**Architecture:** Чистый presentation-layer перенос. FSM (`app.machine.ts`) не трогаем — состояние `menu` остаётся, меняется только его UI. Элементы управления переезжают в `SettingsPage` новой секцией «Тренировка», используя существующие `.field`-идиому, i18n-ключи и контракт-токены. Заодно `SessionDurationSelector` переселяется `train/` → `ui/` (убирает инверсию слоёв). Новый ADR не требуется.

**Tech Stack:** SvelteKit 2 + Svelte 5 (runes), TypeScript strict, i18n через `dictionaries/{en,ru}.json`, Storybook.

**Спецификация:** `docs/superpowers/specs/2026-07-12-first-run-config-to-settings-design.md`

> **Решение владельца (2026-07-12): пояснения ПОКАЗЫВАЕМ, из i18n.** Task 3 рендерит под
> каждым из пяти `Select`-элементов управления пояснение (`<span class="hint">`), используя уже
...
| `src/components/ui/SettingsPage.svelte` | + секция «Тренировка» (6 элементов управления, порядок: Имя → Тема → **Тренировка** → «Назад» → danger) |
| `docs/plans/2026-07-05-mvp-launch.md` | отметка P0-11 (a) → ✅ |

Storybook-истории `SettingsPage.stories.svelte` менять не нужно: секция «Тренировка» не за auth-барьером → рендерится во всех существующих историях автоматически (нужен лишь новый ключ словаря из Task 1). У `MenuScreen` истории нет.
...
Держать **оба** edit'а: паритет ключей `en`/`ru` тестом не подстрахован (`ru.json` не читается ни одним тестом, `Dictionary = typeof en`), ADR 0022 требует обе языковые версии.
...
### Task 2: Свести MenuScreen к CTA + переподключение старта
...
- [ ] **Step 2: Обновить переподключение `onStart` в `MainContent.svelte`**
...
(код в code-fence'ах не трогаем, этот блок содержит код в разметке, но калек там нет)
...
Run: `make check`
Expected: 0 ошибок. svelte-check видит согласованные типы; `FingerLayoutId`/`FlowLineCursorType`/`SymbolLayoutId`/`TextLanguage` разрешаются из `@/interfaces/types`; касты `onChange` валидны против полей `UserSettings`. Импорт `SessionDurationSelector` из `./` разрешается (файл переехал в `ui/`).

- [ ] **Step 7: Визуальная проверка в Storybook**

Run: `make storybook`, открыть `UI/SettingsPage` → историю `Guest`.
Expected: под темой видна секция «Training» с шестью элементами управления (язык текста, раскладка, пальцы, курсор, ритм, длительность-сегменты); каждый с пояснением; danger-зона отсутствует (гость). История `Authenticated` — та же секция + имя + danger.

Проверить edge-case совместимости: сменить «Text Language» на `Russian` → список «Keyboard Layout» перестраивается, а сама раскладка приводится к `йцукен` (нормализация в `settings.ts`; пустой корпус `ru`+`qwerty` из UI недостижим — это ожидаемо, не баг).

- [ ] **Step 8: Commit**

```bash
git add src/components/ui/SessionDurationSelector.svelte src/components/ui/SettingsPage.svelte
git commit -m "feat(settings): секция «Тренировка» — 6 элементов управления на /settings; SessionDurationSelector → ui/"
```
...
Expected: секция «Тренировка» с шестью элементами управления; смена значения любого списка сохраняется (перезагрузка страницы держит выбор — localStorage). Смена «Язык текста» на `ru` перестраивает список раскладок и приводит раскладку к `йцукен` (поведение перенесено дословно).
...
- §2, заголовок пункта **P0-11** — на строке 185; сам пункт **(a)** — ниже (строка 189). В начало абзаца пункта (a) добавить маркер `✅ **Сделано (2026-07-12):**` с одной фразой — «MenuScreen сведён к одиночной CTA; шесть тренировочных элементов управления переехали на /settings новой секцией «Тренировка»; FSM не тронут, ADR не потребовался». Статус пункта **(d)** не трогать — остаётся открытым.
...
- **Логика совместимости** «язык текста → раскладки»: `normalizeSettings` (`settings.ts:88-92`, гоняется на каждом `update`) приводит несовместимую символьную раскладку к дефолту языка. Т.е. смена «Язык текста» на несовместимый **сразу переставит** раскладку — это существующее поведение, оно и защищает от пустого корпуса. Переносим как есть, не «чинить».
