...
- **Helper `parseRootTokens(path, selector)`** живёт в этом же тестовом файле (или `src/themes/__helpers__/parse-css.ts`, если переиспользуется в других тестах). регулярки достаточно — формальный CSS-парсер не нужен (`postcss` не используем):
  ```ts
