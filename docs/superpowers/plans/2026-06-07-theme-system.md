...
**Цель**: курatedный каталог тем (functional) с CSS-only механизмом, без runtime-магии, без build-step и без token pipeline. Принцип — **каждая тема = полный colour snapshot** (semantic + domain colors), primitives неприкосновенны, motion/density — orthogonal axes, реализуемые отдельно.
...
| 7 | **Mirror-key + inline-script для FOUC** | `localStorage['flow-typing-theme']` зеркалит `preferences.theme`. 5-строчный inline-script в `src/app.html` ставит `data-theme` до paint. |
...
| 10 | **`resolveTheme()` + matchMedia listener** | Единственная функция-резолвер. Listener подхватывает смену system-темы при `'auto'`. |
...
- `src/lib/preferences.ts`:
  ...
  - добавить второй `store.subscribe`, зеркалирующий `theme` в `localStorage['flow-typing-theme']` (рядом с уже существующим subscriber на `STORAGE_KEY`). Это **единственный** источник записи в mirror-key — никаких reconciliation-эффектов сверху.
...
- `src/themes/_template.css` — селектор `:root[data-theme="__TEMPLATE__"]` со всеми ~62 color-токенами из base `:root` (значения — `unset` или закомментированные плейсхолдеры). НЕ попадает в bundle (не импортируется). Включается в contract-тест структурно.
...
- `src/themes/dark.css` — первая реальная override-тема. Полный snapshot. `color-scheme: dark;`. Правила трансформации — **ориентир, не точная формула**; итоговые значения подбираются под визуальную читаемость на тёмном фоне и проходят acceptance-контраст. Конкретные значения в snippet'ах ниже — авторитативные:
...
- `src/components/ui/Select.svelte`:
  ...
  - проп `options: Option[] \| OptionGroup[]` — рендеринг определяется через дискриминатор;
  - шаблон:
    ```svelte
...
- `src/app.html` — inline `<script>` **в `<head>` до `%sveltekit.head%`** (выполнится до paint и до hydration):
  ...
  **Порядок гарантии**: inline-script synchronous, выполняется до парсинга `<link rel="stylesheet">` из `%sveltekit.head%` (SvelteKit инжектит CSS-link именно туда). Поэтому к моменту первого CSS-match атрибут `data-theme` уже установлен — стили темы применяются на первом paint без вспышки.
...
- `$effect` #2 — активен только при `$preferences.theme === 'auto'`, с **explicit cleanup** против leak'а listener'а:
  ...
  ```
  Паттерн зеркалит уже использующийся `subscribe`/`unsubscribe` идиом из `TrainingScene.svelte` (см. CLAUDE.md — раздел про подписку на дочерний xstate-actor).
...
- **Helper `parseRootTokens(path, selector)`** живёт в этом же тестовом файле (или `src/themes/__helpers__/parse-css.ts`, если переиспользуется в других тестах). Регекспа достаточно — формальный CSS-парсер не нужен (`postcss` не используем):
  ```ts
...
Внешнее `@media` гарантирует: даже если браузер по какой-то причине запустит transition при reduced-motion, никакой кастомной анимации не будет (system overlay тоже сам мягче по умолчанию).
...
Тип `dictionary` — глобальный `Dictionary = typeof en` (в `src/interfaces/types.ts`). Новые ключи в JSON-словарях подхватываются TS-инференцией автоматически.
...
**Замечание о прокидывании `dictionary`**: проп приходит сверху через `MainContent.svelte` (`<UserPreferencesPage … {dictionary} />`). Тип словаря — глобальный `Dictionary = typeof en` (определён в `src/interfaces/types.ts`); новые ключи в `dictionaries/{en,ru}.json` подхватываются TS-инференцией автоматически. **Не вводить** локальный prop-тип в `UserPreferencesPage.svelte` (он конфликтует с глобальным `Dictionary`). Достаточно убедиться, что ключи добавлены **в оба** словаря (en + ru), иначе TS падёт на пересечении.
...
- В Settings есть выпадайка «Theme» / «Тема» (по `interfaceLanguage`), сгруппированная по colorScheme.
...
- **`color-scheme` валидация**: открыть `<select>` в Dark теме (нашem собственном theme picker и любом других дропдауне) — выпадающая панель браузера тёмная, не белая. Проверить на macOS Chrome и Safari.
...
**Acceptance**:
- В Storybook toolbar появляется выпадайка с темами.
...
- `src/design-system.json` — расширяется новым блоком:
  ```jsonc
  {
    ...
    "themes": {
      "$description": "Full colour snapshots per theme. Domain semantics (finger model, navigation roles, press results) preserved across themes.",
      "light": { /* full colour map, mirrors base :root */ },
      "dark":  { /* full colour map */ },
      "sepia": { /* … */ }
    }
  }
  ```
...
- Claude Design при ingest читает темы как полноценные палитры (sanity-check после деплоя; **не блокирует phase'у**, фиксируется в issue, если AI не подхватил).
...
- Внешний вид свач'ей сохраняется при свитче темы: они **не** управляются токенами текущей темы — это репрезентация **выбираемых** тем, не текущей.
...
| `src/themes/contract.test.ts` падает на любой рассинхронизации (registry ↔ FS, contract violation, отсутствие `light`/`dark`, отсутствие `color-scheme`, дрейф `_template.css`) | автоматически, в нём ≥10 разных assertion'ов |
...
| Native `<select>` dropdown chrome, scrollbars соответствуют colorScheme темы | manual проверка в dark теме — открыть любой select, скроллбар, autofill — должны быть dark, не белые |
...
- [`prefers-reduced-motion` (MDN)](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-reduced-motion) — accessibility media query, гейтит VT animation
- [`color-scheme` CSS property (MDN)](https://developer.mozilla.org/en-US/docs/Web/CSS/color-scheme) — управление chromeом нативных контролов per theme
...
> Историческая справка о том, как план эволюционировал. Все правки **интегрированы в фазы выше** — этот раздел не авторитативен, он только показывает, что и почему менялось, для будущих читателей и для аудита процесса. Если что-то здесь противоречит фазе — побеждает фаза.
...
Два независимых ревью (Plan-agent + general-purpose) нашли 6 блокеров (фактические ошибки путей и API — план был неисполним) + 10 should-fix + 6 расплывчатых acceptance-критериев.

**Блокеры первой волны (B-волна)**:
...
Второй раунд ревью подтвердил, что блокеры сняты корректно, но интеграция открыла **исполнительные** дыры: edge-cases async, sync-source без enforcement, browser-quirks, cleanup-patterns. 6 новых блокеров + 6 should-fix + полировки.
...
Размер плана после v3: 632 строки. Структурно — две Patches-секции в начале как двойной авторитативный источник параллельно с фазами.
...
| T3 | Dark.css правила инверсии vs конкретные oklch-значения — численная нестыковка | переформулировано: «правила — ориентир, итоговые значения в snippet'ах авторитативны» |
...
| T6 | Inline-script без `try/catch` крашится в Safari Private Mode | обёрнут в try/catch с matchMedia-fallback |
...
| T8 | Локальный prop-тип `dictionary` в Phase 1.4 конфликтует с глобальным `Dictionary = typeof en` | удалён manual narrowing; полагаемся на TS-инференцию |
...
- Раунд 1: 6 блокеров + 10 should-fix (план неисполним)
- Раунд 2: 6 блокеров + 6 should-fix (план исполним, но с сюрпризами)
- Раунд 3: 0 блокеров + 4 настоящих fix'а + 7 полировок (план implementable, нужна doc hygiene)
