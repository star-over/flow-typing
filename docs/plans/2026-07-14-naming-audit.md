# План: аудит нейминга — трекинг исполнения
...
- После ренейма Convex-функций/схемы — `npx convex dev --once` для регенерации `_generated` + деплой-валидации.
- Ренейм CSS-контракт-токенов двигать синхронно: темы (`src/themes/*`) + компонент + `*.contract.ts` + агрегат `THEME_CONTRACT`.
...
Детальный self-contained разбор с якорями — во временном handoff той же сессии; ниже — трекинг-срез. При необходимости перед началом — **грилл-сессия** (`grill-with-docs`) для локов терминологии 1.1.
...
- [ ] `npx convex dev --once` — регенерация `_generated` + деплой-валидация
...
- [ ] удалить мёртвый `src/app/` (untracked, пустой, наследие Next.js, коллидирует с `src/components/app/`)
...
- [ ] `src/user-settings/user-settings.ts`→`defaults.ts` (статтер; grep импортёров `DEFAULT_USER_SETTINGS` — радиус шире; можно отложить, если держим волну плоской)
...
- [ ] **`summary`/`payload`** — единое имя `summary` на соседних экшенах (`session.machine.ts:93/96`, `session-impl.ts:64/78`)
...
- [ ] зафиксировать конвенцию `Ms`-суффикса (внутри пайплайна `latencyMedian`/`latencyEwma` без суффикса, на выходе к UI/журналу — с `Ms`) — одной строкой в `CONTEXT.md`
...
- [ ] **investigate** дедуп `RepertoireProgress`/`ProgressionDetail` (дублируют тело подсчёта; второй — почти надмножество) — слить в один источник + derive-селектор, ИЛИ выровнять имена, если payload-экономия намеренная. Возможен ADR
...
- **Источник:** аудит нейминга сессии 2026-07-14 (4 параллельных субагента: term-fidelity, границы модулей, каталоги/файлы, локальные имена + split/merge). Ядро тройки `ladderStep`/`stepLevel`/`openedSteps`, граница `settings-sync↔userSettings`, ViewModel-pipeline и семья `summary` — проверены и **корректны, не трогать**.
