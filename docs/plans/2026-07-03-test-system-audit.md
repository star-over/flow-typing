...
1. **Нет ни одного `setupFiles`** ни в одном из 4 vitest-проектов (`vitest.config.ts`).
   ...
   (`hands-scene.test.ts`, `keyboard-scene.test.ts`). Весь остальной setup — встроенный в
...
не закрываемые без развёртывания реального Convex: ветка `openedSteps > 1` не гоняется ни разу
(обёртка получает openedSteps через `getAuthUserId`, в тестах identity нет → всегда
...
всплывает единственным не тестируемым куском в query. Зарабатывает хлеб.
...
сделано с прочими handler'ами. Обёртка `drillNext` разрешает identity/openedSteps и зовёт ядро.

**Выгода.** Ядро отбора впервые становится тестовой поверхностью: seed-детерминизм,
бюджет, broken-ref fallback **и `openedSteps > 1`** — модулем, без развёртывания. Locality:
...
менялся — извлечение не меняет поведение, сквозные query-тесты зелёные. **Закрыто
покрытие:** два модуль-теста на шов — `openedSteps 2` (шаг 1 впущен, шаг 2 отсечён по bound
...
### 2. `StreamSymbol`/`TypingStream` — у ядра домена нет фабрики-дома: строится тремя несогласованными способами в 9 файлах — HIGH · ✅ СДЕЛАНО

**Файлы:** фабрика `sym()` дословно — `session.machine.test.ts:11`, `training.machine.test.ts:52`,
`session-queue.test.ts:7` · фабрики `press()`+`streamSymbol()` дословно —
`drill-summarize.test.ts:6`, `exposure-reading.test.ts:6`, `session-summarize.test.ts:15` ·
встроенные литералы `{ targetSymbol, targetKeyCaps, attempts }` — `stream-utils.test.ts` (10×),
...
уже держит ровно эти объекты. Override-фабрики payload'ов изобретены заново по файлам:
`stats-calculator.test.ts:6` (`summary()`), `sessions-store.test.ts:4` (`session()`),
`auth-state.test.ts:8` (`mockUser()`); встроенный `UserSettings`/`CloudSettings` —
...
attempts) заставит трогать до 9 файлов; два семейства фабрик уже дрейфуют по стилю
(`sym` vs `press`+`streamSymbol`). Deletion-тест: удали воображаемый фабричный дом — 6 копий
двух вспомогательных функций + встроенные литералы всплывают у 9 вызывающих.

**Решение (направление).** Один тонкий-интерфейсом / много-поведения дом фабрик для тестов
доменных объектов (`StreamSymbol`, `TypingStream`, аккорд `KeyCapId[]`, attempts), плюс
повторное использование готового layout-bootstrap из `test-data.ts`. Возможно, туда же —
override-фабрики payload'ов.
...
**Перед исполнением (сессия вопросов).** Где селится дом (`src/fixtures/` расширить vs новый
`src/test-support/`); одна фабрика с overrides vs набор мелких (`sym`/`chord`/`attempt`);
переносить ли payload-фабрики сразу или отдельным шагом; как не втянуть прод-импорты в
тестовый дом лишнего.

**Сделано (2026-07-03).** Ветка `refactor/stream-test-builders`. Дом фабрик —
...
target, attempts)`. Детализация — набор мелких (не одна `symbol({...overrides})`), чтобы
различия в «идентичных» файлах свелись к «удалить локальный def + импорт». **Объём — только
...
`app.machine.test.ts` (ровно `sym('x','KeyX')`). Классификационные встроенные литералы
(`stream-utils`/`press-result-utils`/`typing-stream`) **оставлены дословно** — там attempts
это предмет теста, а `dev/typing-run` требует `endAt`, которого фабрика `press` не несёт
(ловушка вариативности). Phase 2 из handoff (bootstrap раскладок + payload-фабрики) —
отложена отдельным шагом, чтобы различие остался обозримым. Правки сохраняющие поведение: число
...
`const sym` на :75 — не фабрика, коллизии нет, `sym` туда не импортируется); `StreamAttempt`
несёт ещё `endAt?`.

---

### 3. Обвязка XState-машин: тесты обходят собственный шов `selectors.ts`, тип снимка протекает, тест-родитель изобретён заново 4× — HIGH · ✅ СДЕЛАНО
...
(`app.machine.test.ts:14`) · DI-`provide` встроенный 6× в `session.machine.test.ts`
...
- **Типовая протечка снимка:** `createActor(machine).getSnapshot()` не несёт тип машины
  → приведение типов `as SessionSnapshot` на каждом чтении (37× в одном файле).
- **«Тест-родитель, пересылающий события ребёнку и копящий выходы» написан заново 4 раза**;
...
**Решение (направление).** Тонкий-интерфейсом обвязка машинных тестов: типизированный
читающий метод снимка ребёнка поверх `selectors.ts` (тест пересекает тот же шов, что UI),
повторно используемая фабрика «тест-родитель/sink», общий фейк-провайдер сессии; протечку приведения типов снимка
закрыть на уровне обвязки.

**Выгода.** Leverage: знание про invoke-id/children — в одном доме (совпадает с
`selectors.ts`), рефакторинг id внутри машин не рушит тесты. Locality: 37 приведений типов → одно.
Меньше boilerplate на кейс.

**Перед исполнением (сессия вопросов).** Почему `getSnapshot()` теряет тип (generics
`createActor` vs конкретная машина) — лечится ли это на уровне обвязки без `as`; тянуть
ли `selectors.ts` в тесты напрямую или дать тест-обёртку над ним; сводить ли 4
тест-родителя в один параметрический строитель (риск — потерять читаемость каждого стыка).
Иерархию трёх машин (ADR 0007) не трогаем. Пара к кандидату 6 (fake timers).

**Сделано (2026-07-03).** Ветка `refactor/machine-test-harness`. Дом обвязки —
...
под `svelte-check`): 34 `as SessionSnapshot` — избыточный шум, а не отсутствие помощника —
...
сужен по итогам grilling (Опция B — против пере-абстракции):** 4 тест-родителя в один
...
Поведение сохранено — 12 исходных session-тестов целы, все 54 файла зелёные (**504
инвариант рефакторинга**). Проверено: `make check-dev` (eslint/svelte-check чисты);
...
создаётся per-test (25× в `drill.test.ts`) · `insert('users', …)` — 16× (`drill.test.ts`),
...
дублированием в агрегат `drillIndex`, `drill.test.ts:32`), «поднять профиль» — дублирование
...
**Решение (направление).** Дом фикстур сева для проекта `convex`: фабрики
`seedUser`/`seedDrill` (с дублированием агрегата)/`seedProfile` + обёртка,
...
новый backend-тест не переписывает сев. Тесно связан с кандидатом 5 (тот же обвязка
добавит `withIdentity`).

**Перед исполнением (сессия вопросов).** Насколько фабрики знают про инвариант агрегата (сев
drill сам дублирует в `drillIndex` — тогда `insertDrill` не обходит `insertBatch`); давать
...
**Файлы:** grep `withIdentity` по репозиторию — **0 вхождений**. У всех обёрток (`getMine`,
...
**Перед исполнением (сессия вопросов).** Что реально проверяем в authenticated-ветке (что обёртка
разрешает id и делегирует — vs дублировать проверки handler'а); порядок — сперва seed-дом
(4), потом это.
...
`useFakeTimers`/`advanceTimersByTime`/`setSystemTime` — 0 вхождений в репозиторий. Смежно:
...
`+20`, бьёт 6 нажатий) — смена константы рассинхронизирует «6 нажатий < порог < длина» и лок
посыплется молча.
...
**Перед исполнением (сессия вопросов).** Уживётся ли `vi.useFakeTimers` с `fromPromise`-актором
(`fetchDrills`) и `vi.waitFor` (Promise и фейковые таймеры — типичная ловушка); стоит ли
покрывать реальный `TICK` или достаточно `isExpired`-модуля (тот уже покрыт как чистая fn в
`session-timer.ts` — прошлый аудит, кандидат 6); только точность или ещё развязка от
...
сплющивает микрозадач `fetchDrills` до `armed` → `KEY_PRESS` (старт тикера) →
`advanceTimersByTimeAsync(> окна)` → `done`; `vi.waitFor` в этих тестах **не** используется
(детерминированный продвижение вместо polling'а — конфликт с фейковыми таймерами исключён).
...
(формула обратного отсчёта; edge-обнаружение «вход в training»; порядок registerBeat +
...
(как прежде), руны оставить тонкими. Не всё вынашиваемо (DOM-побочные эффекты, rAF) — только логика.

**Перед исполнением (grilling).** Что реально кристаллизуемо (формула таймера и edge-обнаружение
— да; auth-rewire и DOM-побочные эффекты — вряд ли); стоит ли трогать `registerBeat` (модель здорова,
...
автоматической проверки (даже a11y-дополнение работает лишь в интерактивном UI). Развилка:
...
- **`hands-scene.test.ts:27–188` — 15 почти идентичных `it()`-блоков** (один и тот же
  ...
  механический. (14 фикстур — замороженные снимок-литералы без генератора: правка формы
...
- **Coverage-конфиг** (`vitest.config.ts:40–45`): exclude чистый (ничего важного не
  ...
  форсится ни в одном барьере (`check-all` без него; `reporter: ['text']` только).
...
- **Чистые `.ts`-модули с логикой без теста** (тестируемы в node прямо сейчас):
  ...
  `src/interfaces/types.ts:290–304` (runtime-zod `.refine`/`.superRefine` «не больше одной
...
| 2 | `StreamSymbol`-фабрика-дом | HIGH | углубление (тест-инфраструктура) |
| 3 | Обвязка XState-машин | HIGH | углубление (тест-инфраструктура) |
| 4 | Convex-сев — дом фикстур | MEDIUM-HIGH | углубление (тест-инфраструктура) |
...
**Естественные пары для одной сессии:** 3+6 («машинный обвязка + fake timers» — одна зона),
4+5 («convex-обвязка: сев + identity» — тот же обвязка). 1 самостоятелен и, возможно, самый
ценный (единственная реальная shallow→deep в production + чинит корень дыр покрытия).
