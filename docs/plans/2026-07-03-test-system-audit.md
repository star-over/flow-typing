...
(обёртка разрешает openedSteps через `getAuthUserId`, в тестах identity нет → всегда
...
уже держит ровно эти объекты. Override-билдеры payload'ов переизобретены по файлам:
...
### 8. Storybook — ручная витрина: 12 компонентов получают ноль авто-проверок — MEDIUM · ⛔ ПРИНЯТ КАК ЕСТЬ (ADR 0013)
...
автоматической проверки (даже a11y-расширение работает лишь в интерактивном UI). Развилка:
...
**Решено (2026-07-03).** Пользователь выбрал вариант «а» — принять витрину как есть.
Зафиксировано **ADR 0013** «Storybook — визуальная витрина без авто-проверок»
(jsdom/browser-mode в CI не вводим; логика вынесена за чистые швы и покрыта в node;
витрина — предмет ручного визуального ревью). Ветка `chore/audit-hygiene-tails`.

---

## C. Мелочи и гигиена (не отдельные сессии)

**Сделано (2026-07-03).** Ветка `chore/audit-hygiene-tails` (грилл подтвердил подмножество
и развилки C3/C4). Число тестов растёт за счёт C4 (+32), C1 — инвариант.
`make check-dev` зелёный (eslint/svelte-check/vitest: 60 файлов, 568 тестов). Convex не
тронут. Закрыто:
- **C1** — `hands-scene.test.ts`: 15 развёрнутых `it()` → один `it.each` поверх барреля
  фикстур `src/fixtures/hands-scene/index.ts` (ре-экспорт + помеченная таблица
  `handsSceneFixtures`). 15 кейсов инвариантны; блок `sealHandsSceneViewModel` не тронут;
  метка-опечатка `"Sift-N"` → `"Shift-N"`.
- **C2** — `vitest.config.ts` coverage exclude += `src/lib/dev/**` (dev-инструментарий,
  как `scripts/**`), `src/data/**` и `src/**/*.contract.ts` (данные, не логика). Coverage
  нигде не гейтится — правка снимает структурное занижение %.
- **C3** — **ADR 0014** «Форма записи символьной раскладки объявлена трижды — ради развязки
  shared↔src». Развилка решена **против** унификации: полный единый владелец невозможен без
  слома активного инварианта «`src` не импортирует `shared`» (0 вхождений, комментарии
  запрещают) либо конвенции «`types.ts` = единый язык с JSDoc». Три объявления служат трём
  мастерам (серверно-tooling-форма / доменный словарь / рантайм-гейт).
- **C4** — покрыт расширенный набор чистых модулей: `drill-utils` (11 fn), `state-utils`
  (`inState`), `layout-utils` (`createKeyCoordinateMap`), `types.ts` zod-refine/superRefine
  (дескриптор + реестр раскладок).
- **C5** — CLAUDE.md тест-раздел: 2 проекта → 4 (`shared`, `auto-flow` добавлены). Дрейф
  `draining` в CLAUDE.md/CONTEXT.md уже был снят в кандидате 6; дополнительно поправлен
  всплывший дрейф `docs/05-adaptive-learning-system.md:82` (описывал `draining` как текущее
  поведение — противоречит ADR 0007).
...
| 8 | Storybook авто-проверки | MEDIUM | ⛔ ПРИНЯТ КАК ЕСТЬ (ADR 0013) |
