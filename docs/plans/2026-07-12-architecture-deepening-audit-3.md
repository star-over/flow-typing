...
**Сделано (2026-07-13).** Ветка **(A) required + seal-enforced** исполнена (general-purpose-агенту
передан кристаллизованный шов). Контракт ViewModel расширен: на TARGET-пальце —
...
комментарий — граф в компоненте больше не пересобирается (был **вторым** построением графа поверх
...
`simple_t=['KeyF','KeyR','KeyT']`, вырожденный `simple_k=['KeyK']`, многоклавишный TARGET `shift_f` =
...
`it.each`-фикстур не изменилось). `make spell` зелёный (кальки переписаны в источнике; недостающие склонения доменных форм — в
`.cspell/project-words.txt`). Ветка deep-module-аудита.
...
### 7. Осиротевший предикат `isSymbolReady` — хвост консолидации (холистическая ревизия) — LOW · ✅ СДЕЛАНО

**Найдено комплексной приёмкой всего аудита-3 (2026-07-13).** Не из первичного списка кандидатов —
всплыло при взгляде на результат в целом. После кандидата 5 (`evaluateStepReadiness` вобрал обход
готовности) `growth.decideOpenedSteps` перестал звать `isSymbolReady`; предикат осиротел (0
прод-потребителей, только собственный тест). Кандидат 5 пометил его «на будущий проход» — холистическая
ревизия и есть это будущее.

**Файлы:** `shared/repertoire/readiness.ts:66` (`isSymbolReady`) + 7 кейсов `describe('isSymbolReady')`
в `readiness.test.ts`.

**Проблема.** Мёртвый предикат с зелёным тестом = ложная locality (читается как живое Readiness-API) —
тот же класс, что 1A/1B. Deletion-тест: удали → сложность исчезает (только собственный тест).
`readinessGaps` при этом жив — его напрямую держит `evaluateStepReadiness` (`readiness.ts:99`).

**Сделано (2026-07-13).** Удалён `isSymbolReady` (12 строк) + его `describe` (7 кейсов); импорт в
`readiness.test.ts` сужен. `readinessGaps`/`evaluateStepReadiness` не тронуты (re-grep подтвердил —
`readinessGaps` жив через `evaluateStepReadiness`). Верификация: re-grep `isSymbolReady` пуст,
`make check-dev` зелёный (vitest **643 теста** — −7 кейсов от 650), `make spell` 0. Ветка
deep-module-аудита.
