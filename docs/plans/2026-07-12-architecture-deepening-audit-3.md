...
### 1. Уборка мёртвого и продублированного — HIGH (navigability) · LOW (риск) · ☐ не начато
...
#### 1B. Предикат «совместимость раскладки и языка drill'а» — мёртвый дом + инлайн-дубль дважды · ☐ не начато
...
### 2. `-utils`-мусор-баги → концепт-модули — MEDIUM–HIGH · ☐ не начато
