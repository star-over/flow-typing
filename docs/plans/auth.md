...
- [x] Решить: имя проекта в Convex — `flow-typiing` (typo, не блокер); deployment `wandering-ocelot-9` (EU-West-1)
...
- [x] `.env.example` лежит в репо, `.env.local` — нет
...
- Domain verification — два варианта:
  - (a) свой production-домен с DNS-контролем + production cloud Convex deployment (классический путь);
  - (b) попытаться верифицировать сам `*.convex.site` через Apple's domain association file (Convex HTTP routes могут отдать `apple-developer-domain-association.txt` на нужном пути). Не подтверждено; проверять отдельно. Если получится — Phase 9 делается на текущем dev cloud deployment'е.
- Если (b) не работает — production-deployment Convex с production `SITE_URL` и собственный домен.

**Cloud-mode уже даёт HTTPS** (`https://wandering-ocelot-9.eu-west-1.convex.site`) — это снимает HTTP-блокер local-mode, но не освобождает от Apple's требования к Return URL domain verification. Конкретный workflow выбирать в Phase 9.
