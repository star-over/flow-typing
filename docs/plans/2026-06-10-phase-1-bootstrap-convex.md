...
**Goal:** Convex backend подключён к FlowTyping end-to-end: схема + health-функции задеплоены в local Convex, клиент инициализирован из SvelteKit, диагностическая страница доказывает работу connection.
...
Прежде чем стартовать первый таск:
...
- [ ] **Стартовать с чистого `master`.** Сначала проверить, что текущий стейт чистый и `master` существует, потом создать ветку:
  ```bash
...
- [ ] **Запустить watcher в dedicated терминале.** Открыть **новый таб/окно терминала** и пометить его как `convex-watch`. В нём:
  ...
  npx convex dev
  ```
  Это watcher: при каждом изменении файла в `convex/` синкает к cloud deployment'у (`wandering-ocelot-9.eu-west-1.convex.cloud`) и регенерирует `convex/_generated/`. **Все последующие таски предполагают, что этот процесс живёт.** Не закрывать таб и не делать `Ctrl+C` до Step 8.6. Все остальные команды плана — в другом терминале.
...
Действия:
- Старые строки `NEXT_PUBLIC_CONVEX_URL=...` и `NEXT_PUBLIC_CONVEX_SITE_URL=...` — **удалить**, не оставлять дубли. Если оставить оба префикса, `resolveEnvVarName` детектит `matching.length > 1`, логирует «cannot update automatically», и любая последующая попытка CLI трогать env-файл no-op'ит.
...
```bash
git add .env.example
git diff --staged   # убедиться, что в стейдже только .env.example
...
- [ ] **Step 2.2: Задеплоить схему и проверить регенерацию типов**
...
`npx convex dev` (без `--once`) должен быть запущен с Pre-flight шага про watcher — он синкает к cloud и регенерирует `_generated/`. Проверить:
...
**Цель:** один singleton `ConvexClient`, который импортируется по алиасу `@/lib/convex`. Re-export `api` оттуда же — чтобы компоненты не делали относительные импорты в `../../convex/_generated`.
...
**Production-guard:** используем `import { dev } from '$app/environment'` — SvelteKit статически вырезает dev-ветку из production-бандла (`dev` заменяется на литерал `false` на этапе сборки, template-условие исчезает целиком). Это **корректный tree-shake**, в отличие от `import.meta.env.DEV` в шаблоне (последний только скрывает контент в runtime, но markup остаётся в бандле).
...
**Замечание про scope:** `make spell` сканирует `src/**/*.{svelte,ts,css}`, `dictionaries/*.json`, `static/*.html`. Файлы `convex/**/*.ts` и `CLAUDE.md` в scope **не входят**, поэтому слова в них cspell не проверяет напрямую. Если решишь расширить scope spell — это отдельный мини-таск, в эту фазу не пихай.
...
Если `set` или `list` упали — записать симптом в заметник. **Fallback для Phase 2** (если `npx @convex-dev/auth` не запустится автоматически):
...
2. **Создать руками** файлы, которые CLI обычно скаффолдит: `convex/auth.config.ts`, `convex/auth.ts`, `convex/http.ts` — шаблоны на https://labs.convex.dev/auth/setup/manual.
...
- [ ] **Step 8.5: Зафиксировать значения для Phase 2 (в локальный заметник, не в репо)**
...
| `SITE_URL` | `http://localhost:5173` (dev) или production-домен фронта | **Convex backend env** (`npx convex env set SITE_URL ...`) | куда Convex Auth редиректит браузер после успешного логина (Vite в dev, production-домен в release) |
...
**Триплет.** `PUBLIC_CONVEX_SITE_URL` — для фронта; `SITE_URL` — куда Convex редиректит после логина (Vite origin в dev); `CONVEX_SITE_URL` — issuer-URL для self-issued JWT. **Особое внимание:** не делать `npx convex env set CONVEX_SITE_URL ...` — это перетрёт автозначение cloud-deployment'а.
...
После того как `master` зеленый и up-to-date — merge с явным merge-commit'ом (`--no-ff` сохраняет «это была фича-ветка» в истории):
...
- [ ] `.env.example` лежит в репо с актуальными ключами, `.env.local` остаётся gitignored
...
- **Пререкизит Phase 2:** `npx convex dev` (или `make convex`) должен быть запущен в отдельном терминале при выполнении `npx @convex-dev/auth` setup-команды (она внутри ходит к cloud backend через `npx convex env set`).
...
3. **Type consistency:** `api.health.ping`, `api.health.tick`, `ConvexClient`, `PUBLIC_CONVEX_URL` — единообразно через все таски.
