...
- **Refill стадируется:** первый fetch под бюджет окна уже даёт текста ~на сессию. Задачи 1–6 дают играбельную таймерную сессию **без** refill; задача 7 добавляет добор + адаптацию mid-session.
...
- `src/machines/session-impl.ts` — реальные реализации `fetchDrills` (локально → сервер) и `recordCheckpoint` (skip → Convex) + экспорт `sessionService = sessionMachine.provide({...})`. Здесь живёт импурность.
...
> Примечание: `MainContent`/`TrainingScene` ещё используют `currentSymbolLayoutId` из training-контекста — он сохранён, всё ок. Ссылка `state.children.trainingService` в `App.svelte` пока валидна типово (training всё ещё прямой ребёнок до задачи 4).
...
Чистая машина одного прогона: `loading` (первый fetch) → `active` (`running`/`paused`/`draining`, invoke training) → `done` (финальный чекпоинт + `SESSION.COMPLETE`). Провайдеры `fetchDrills`/`recordCheckpoint` инъектируются.
...
> `TIMER_EXPIRED` — внутреннее событие, которое в проде шлёт тикер при `displayElapsedMs >= окно`; в тесте подаём напрямую, чтобы не гонять реальные таймеры.
...
2c. Удалить ассерты построения потока (поток теперь строит сессия, не корень):
- в тесте «from menu: enters training.running…» (текущие строки 27-28) убрать:
  ...
  ```
- в тесте «Enter NAVIGATION_KEY restarts training…» (текущие строки 145-146) убрать те же два `lastTrainingStream`-ассерта — здесь они падают по иной причине: после рестарта `lastTrainingStream` равен `[]` предыдущей сессии (из `arriveInSessionComplete` со `stream: []`), а не «поток не построен».
...
> `exit` на `paused` шлёт `RESUME_TIMER` и при `RESUME`, и при возврате в `running` через Enter. Уход в `#app.menu` тоже триггерит `exit` — это безвредно (session вот-вот будет остановлена сменой состояния `training`). Если потребуется точечность — перенести `RESUME_TIMER` на входы целевых `running`. Для простоты оставляем на `exit`.
...
`src/lib/dev/typing-capture.ts` — забытый потребитель: подключён в `appActor.ts` под `import.meta.env.DEV`, читает `state.children.trainingService` (станет `undefined` — training теперь внук) и гейтит по `value === 'lessonComplete'` (состояние снято в задаче 2). Без миграции `make check` краснеет ВНЕ трёх UI-файлов.
...
> `.timer`/`.state-code` уже застилизованы; класс `state-code` переиспользуем. Обновить устаревший комментарий в `src/components/ui/TrainingScene.contract.ts` (ссылается на `trainingState.value` в `<code>`, теперь там таймер).
...
> Если на старте мелькает пустой кадр (session в `loading`) — это ок (training-ребёнка ещё нет); поток появляется после первого fetch. При желании добавить плейсхолдер — отдельная мелкая правка.
...
Серверный fetch — побочный (Convex), его не юнит-тестируем сетью. Тестируем только маппинг ответа в склейку — вынесем чистый шаг:
...
Smoke: войти (GitHub/Google) → тренировка → по завершении в Convex-дэшборде `skillProfiles` появилась/обновилась строка для юзера; гостем — тренировка работает, `skillProfiles` не пишется (`console.warn`).
...
> Этот тест с 1-символьной порцией срабатывает при любом разумном пороге — он НЕ проверяет точную границу (guard читает `completed` до push, поэтому `needsRefill` использует `+1`-поправку, см. 3b). Чтобы запинить именно границу, добавь отдельный тест: начальная порция длиной ровно `REFILL_THRESHOLD_SYMBOLS + 2`, печатай по одному символу и проверь, что `refilling` НЕ наступает до порога и наступает на нём — так `+1`-поправка не «починится» обратно в баг при будущем рефакторинге.
...
**Draining (допечатка)**:
Фаза после истечения таймера: дозагрузки нет, очередь допечатывается, затем конец (с кап-драйном — не зависает).
```
...
Run: `make spell`
Если красный — `/fix-spell` (Haiku разберёт по правилам CLAUDE.md; кальки переписать, не вайтлистить).
...
1. **Значение кап-драйна `DRAIN_CAP_MS`:** зависание закрыто (кап + `allTyped` в `draining`, задача 3); открыт лишь подбор числа (сейчас 4с) — сколько давать допечатать хвост после истечения таймера. Настроить после ручной обкатки задачи 5.
...
3. **Стартовый плейсхолдер `loading`:** мелькающий пустой кадр до первого fetch. Если мешает — добавить явный «собираем порцию…» (мелкая UI-правка).
