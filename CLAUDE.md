...
- `sessionMachine` (`src/machines/session.machine.ts`) — invoked в state `training`. Таймерный жизненный цикл сессии: `loading` (первый fetch) → `active` (`armed`/`armedPaused` → `timing` {`running`/`refilling`} / `paused`) → `done`. Собирает порцию drill'ов (`fetchDrills`), склеивает в непрерывный `TypingStream`, invoke'ит `trainingMachine` как внука, накапливает event-sourced проекцию `completed[]` из `TYPING.ADVANCED`, на чекпоинтах (перед дозагрузкой и в конце) вызывает `drillRecord` через `recordCheckpoint`. Провайдеры внедряются через `session-impl.ts`. По истечении таймера сессия сразу завершается (`done`): финальный чекпоинт + каноническая сводка, затем `SESSION.COMPLETE` родителю — без добора очереди (ADR 0007).
