...
*   **Явное сужение типов событий:** Для обеспечения типобезопасности в среде этого проекта, всегда используйте явные проверки-гарды (например, `if (event.type === '...')`) внутри действий (`actions`) и условий (`guards`). Это гарантирует корректное сужение типа `event`. Не следует полагаться на неявное сужение типа, исходящее из определения перехода, так как это может приводить к ошибкам в различных средах разработки.
