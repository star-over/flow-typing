...
*   **Карта движения:** Внутри блока статично подсвечиваются три типа клавиш для наглядности:
    *   `HOME`: "Домашняя" клавиша для данного пальца.
    *   `TARGET`: Целевая клавиша, которую нужно нажать.
    *   `PATH`: Клавиши, лежащие на пути от `HOME` к `TARGET`.
...
3.  **Многоуровневая обратная связь при ошибках:**
    ...
    *   **Неправильная клавиша (в зоне другого пальца):** Это ключевой момент. Система не может физически отследить палец, но она может определить, какой палец *должен был* нажать ошибочную клавишу согласно схеме `FingerLayout`. Например, если цель 'e' (средний палец), а пользователь нажимает 'w' (клавиша безымянного пальца):
        *   Система подсвечивает **правильный палец** (средний) зеленым.
        *   Одновременно она подсвечивает **палец, ассоциированный с ошибкой** (безымянный), красным.
...
Этот рабочий процесс подтверждает, что ядро концепции — это обучение правильной технике и мышечной памяти, а не простому попаданию по клавишам.
