...
Все 22 контракта агрегируются в `src/themes/contract.ts → THEME_CONTRACT` (228 токенов). Контракт-тест `src/themes/contract.test.ts` enforce-ит, что каждая тема (`src/themes/<id>.css`) и `_template.css` декларируют каждый токен; значения свободны.
