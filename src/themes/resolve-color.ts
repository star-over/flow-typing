 ...
 * светлоту ниже нуля, станут в файле снапшота неотличимы друг от друга — это ложная
...
/** Регулярное выражение `CALC` допускает только один символ из `[lch]`; сузить строку без `as`-приведения. */
...
export function formatOklch({ l, c, h, alpha }: Oklch): string {
  // При нулевой хроме цвет не имеет оттенка — оттенок не влияет на отрисовку, но разные
  // ядровые токены дают разные значения `h` (0, 20, 250…). Файл снапшота сравнивает строки,
  // поэтому не канонизировать оттенок означало бы ложную красноту на цвете, который
  // фактически не изменился.
  //
  // Инвариант канонизации: она верна для цвета как отображаемого значения, но не
  // как базы вывода. Если появится роль, выводящая хрому от базы без оттенка
 ...
 * `background-color` с альфой так не считает — наложение CSS/Canvas выполняется
 ...
 * линейный sRGB → гамма-кодировать (`srgbEncode`) → смешать по альфе каналу за каналом
