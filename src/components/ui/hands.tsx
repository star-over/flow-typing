/**
 * @file Компонент `Hands` для визуализации положения рук и пальцев на клавиатуре.
 * @description Этот компонент отображает графическое представление рук, где каждый
...
/**
 * Варианты стилей для рук и пальцев на основе CVA.
 * Определяет цвет каждой части руки в зависимости от ее состояния (`FingerState`).
 */
...
/** Пропсы для компонента `Hands`. */
export type HandProps = React.ComponentProps<"div">
  & VariantProps<typeof handsVariants>

/**
 * Компонент `Hands` визуализирует стилизованное изображение рук и пальцев.
 * Состояние каждого пальца (L1-L5, R1-R5) и оснований кистей (LB, RB)
 * определяется соответствующими пропсами типа `FingerState`.
 * @param props Пропсы компонента, включая состояние каждого пальца.
 * @returns Элемент JSX, отображающий руки.
 */
export function Hands(
  { className, L1, L2, L3, L4, L5, LB, R1, R2, R3, R4, R5, RB, ...props }: HandProps
): JSX.Element {
  return (
    <div
      className={cn(
        handsVariants({ L1, L2, L3, L4, L5, LB, R1, R2, R3, R4, R5, RB, className })
        , "flex w-screen justify-center border-2 border-amber-950"
      )}
      {...props}
    >
      <svg
