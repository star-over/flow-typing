...
## User actions (`user-actions/`) — клавиатурные действия

Единый реестр пользовательских действий (ADR 0032): `user-actions.ts` — данные (действие = `{ id, trigger, when, run }`; триггер строго 1:1 — аккорд `binding` ИЛИ голая клавиша `key`, без альтернатив), `dispatch.ts` — распознаватель по `event.code` (mod = `metaKey || ctrlKey`; фильтры repeat/AltGr/editable target; гейт `when` по состоянию `appMachine`). Точка встраивания — `handleKeyDown` в `src/routes/+layout.svelte` до `appActor.send`: аккорды и голые Escape/Enter идут одним каналом, `NAVIGATION_KEY` в машинах упразднён — они получают только готовые `PAUSE`/`RESUME`/`START_TRAINING`. Визуальные подсказки (`ui/KeyHint`) и `aria-keyshortcuts` читают триггер из того же реестра через `keyHintPropsForTrigger` / `formatAriaTrigger` (`platform.ts`) — расхождение отображения и поведения исключено структурно. Каталог — исключение из правила «логика — плоско»: два файла образуют неделимую пару данные+диспетчер; функции форматирования платформы (`platform.ts`) лежат плоско, как соседи.
