...
7.  **`StreamAttempt` (Попытка):** Запись одного нажатия клавиши пользователем для конкретного символа. Содержит `typedKey` (объект `TypedKey`, описывающий нажатую клавишу), `startAt` (время начала нажатия) и `endAt` (время окончания нажатия).
8.  **`TypedKey` (Нажатая клавиша):** Представляет собой точное описание клавиши, нажатой пользователем. Содержит:
    -   `keyCapId`: Уникальный идентификатор физической клавиши (например, `KeyA`, `Space`).
    -   `shift`: Булево значение, указывающее, была ли нажата клавиша `Shift`.
    -   `isCorrect`: Булево значение, указывающее, было ли это нажатие корректным для текущего целевого символа.
9.  **`StreamSymbol` (Символ потока):** Расширенное представление символа из `Stream`, включающее `targetSymbol` (целевой символ) и массив `attempts` (историю всех попыток набора этого символа).
...
- **Макет клавиатуры (`KeyboardLayout`):** Схема расположения, формы и размера физических клавиш (например, ANSI, ISO). Это базовый "холст" клавиатуры.
  - **Реализация:** `src/data/keyboard-layout-ansi.ts`, тип `KeyboardLayout`.
...
- **Код клавиши (Key Event Code / `keyCapId`):** Уникальный идентификатор физической клавиши, соответствующий `KeyboardEvent.code` (например, `KeyA`, `ShiftLeft`).
  - **Реализация:** `src/interfaces/key-cap-id.ts`.
...
- **Символьный макет (`SymbolLayout`):** Логическая схема распределения символов по физическим клавишам (например, QWERTY, Dvorak).
  - **Реализация:** `src/data/symbol-layout-en-qwerty.ts` (пример), тип `SymbolLayout`.
...
- **Рука (Hand):** Понятие руки используется для группировки пальцев. Отдельного типа `HandSide` в коде нет, принадлежность пальца к левой или правой руке определяется по префиксу идентификатора пальца (например, `L1` для левой руки, `R1` для правой).

- **Палец (FingerId):** Идентификатор пальца с уникальным кодом (L1-L5, R1-R5).
  - **Соответствие пальцам:**
    - L1/R1: Большой палец (Thumb)
    - L2/R2: Указательный палец (Index)
    - L3/R3: Средний палец (Middle)
    - L4/R4: Безымянный палец (Ring)
    - L5/R5: Мизинец (Little)
    - LB/RB: Основание большого пальца (Thumb Base)
  - **Реализация:** Тип `FingerId` в `src/interfaces/types.ts`.
...
- **Пальцевый макет (`FingerLayout`):** Схема ответственности каждого пальца за определенные физические клавиши.
  - **Реализация:** `src/data/finger-layout-asdf.ts`, тип `FingerLayout`.
