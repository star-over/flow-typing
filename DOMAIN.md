...
7.  **`StreamAttempt` (Попытка):** Запись одного нажатия клавиши пользователем для конкретного символа. Содержит `typedSymbol` (нажатый символ), `startAt` (время начала нажатия) и `endAt` (время окончания нажатия).
